...
function handleHtmlBlock(state, token, tokens) {
...
export function parse(schema, content) {
    if(typeof content === 'string') {
        ...
        parser.tokenHandlers.html_block = handleHtmlBlock;

        const node = elementFromString(content);

        let parsableContent = '';
        node.childNodes.forEach(node => {
            if (node.nodeType === Node.TEXT_NODE) {
                parsableContent += node.textContent;
            } else if (node.nodeType === Node.ELEMENT_NODE) {
                const tokens = tokenizer.parse(node.outerHTML, {});
                if (tokens[0].type === 'html_inline') {
                    parsableContent += node.outerHTML;
